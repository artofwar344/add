...
import {showLoading,hideLoading} from '@/utils/loading'
  export default {
    ...
    methods: {
      ...
      submitAdd() { 
        ...
        this.$http.get("api/portal/oss/token" ).then(res => {
          ...
          for (var i = 0; i < fNum.length; i++) {
            ...
            if (Name !=='用户补钱模板.xlsx') {
              that.$message.error("请上传（用户补钱模板.xlsx）文件");
              hideLoading()
              return false;
            }
      ...
      xmls(){ 
        this.$http.post('api/portal/batchGive/batchGiveUserReadMoney?editorName=luweipeng&filePath='+this.Value4,{params: this.$data.search}, {responseType: 'arraybuffer'}).then((res) => {
          console.log(res, '22');
      ...
      download(data) {
        ...
        link.href = url
        link.setAttribute('download', '用户补钱模板.xlsx')
