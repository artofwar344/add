<template>
    ...
    <div class="example-upload">
          ...
          <div class="example-upload">
            <!--自定义上传图片-->
            <Upload
              action="https://download.yn.gov.cn/image-service/uploadImage?bucketId=GSP_PRIVATE&"
              :data="cdnParams"
...
export default {
  ...
  data() {
    return {
      ...
      //上传相关
      showList: false,
      preUrlFixed:"https://download.yn.gov.cn/image-service/downloadImage?bucketId=dangqun&ObjNm=", //图片回显路径
      preUrl: "",
      cdnUploadTokenObj: {},
      cdnParams: {},
      cdnHost: "",
  ...
  methods: {
    ...
    initCDNToken() {
      this.cdnUploadTokenObj.cdnUploadUrl ="http://119.36.239.138:8088/image-service/uploadImage?backetId=dangqun1";
    ...
    uploadLists() {
      ...
      this.$ajaxRequest("post", "/db/material/commit", setdata).then(res => {
          console.log(res);
          this.one = true;
          //裕农通APP功能，勿删！
