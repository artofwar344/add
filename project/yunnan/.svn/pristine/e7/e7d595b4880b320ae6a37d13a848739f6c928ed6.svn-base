    ...
    methods:{
      ...
      $('.hui-popup').on('touchmove',function(event){
        ...
        selectedTaiType(selectedVal){
            const _this = this;
            if(selectedVal[0].text == "团体旅游"){
                  _this.taiNumPicker[0].values = [{
                        text:'六个月一次',
                        value:'1'
                    }]
        ...
        goto(){
           ...
           if(this.selectApply != "单独申请证件"){
                if(this.taivalue.length <= 0){
                    this.$toast("请选择前往地点")
                    return
                }
                if(this.selectTaiType == "" ||  this.selectTaiType == undefined){
                    this.$toast("请选择签注种类")
                    return
                }
                if(this.selectTaiNum == "" ||  this.selectTaiNum == undefined){
                    this.$toast("请选择有效次数")
                    return
                }
