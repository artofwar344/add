<template>
	<div class="listTop">
		<towtop :name="titlename"/>
       	<!-- <div class="listOne1" @click='tolink(item)' v-for="item in datatext" :key="item.guid">
       	   <p>{{item.naAppName}}</p>
           <h1>{{item.publishingTime}}</h1>
       	</div> -->
        <div class="bidding_main">
          ...
          <ul>
            <li @click='tolink(list)' class="bor_b bidding_list" v-for="list in datatext" :key="list.guid">
              <p class="dis_flex margin_b16"><span class="flex_1 corporate_name">矿山/项目名称：{{list.naAppName}}</span>
              </p>

              <!-- <p class="dis_flex font-24 margin_b16">
                <span class="col_999 flex_1 tx_left">宗地编号：{{list.jyptId}}</span>
              </p> -->
             <p class="dis_flex font-24 margin_b16"><span class="col_999 flex_1 tx_left">发布时间：{{list.publishingTime}}</span>
              </p>
              <p class="dis_flex "><span class="col_fb " v-if="list.type=='tkq'">产权类型：{{typeClass[0]}}</span>
...
export default {
  ...
  methods: {
    ...
    loadBottom() {
      ...
      this.$ajaxRequest("post", "/gsp/mng30039", setdata, pagejump).then((res) => {
    ...
    tolink(item) {
      ...
      this.$ajaxRequest("post", "/gsp/mng30040", setdata)
        .then(res => {
          this.$router.push({
            name: "miningThree",
            params: { datatext: res }
