<template>
    ...
    <Loadmore  :bottom-method="loadBottom"
        ...
        <div class="listOne" @click='tolink(list)' v-for="list in datatext"
        style="padding:0.2rem;background-color: white; text-align: left">
            <p>项目名称：{{list.landProjectName}}</p>
            <h1 >宗地编号：{{list.landCode}}</h1>
...
export default {
  ...
  methods: {
    ...
    loadBottom() {
      ...
      this.$ajaxRequest("post", "/gsp/mng30043", setdata, pagejump).then((res) => {
    ...
    tolink(item) {
      ...
      this.$ajaxRequest("post", "/gsp/mng30044", setdata,pagejump)
        .then(res => {
          this.$router.push({
