...
export default {
  ...
  methods: {
    ...
    liveface() {
      ...
      window.sessionStorage.setItem("ApplyReasonType", "02");
      this.saveAllData();
      // console.log(this.$store.state.publicSecurityStore.formData,this.uploadFile)
    ...
    namekeystr(index) {
      if (index < 1) {
        this.arr[index].namekey =
          ...
          "_hz-13_02_abcd00089_" +
          this.namekeycode;
        this.fileOldName = this.arr[index].namekey;
      } else {
        this.arr[index].namekey =
          sessionStorage.getItem("WorkId") +
          "_hz-13_02_abcd00085_" +
          this.namekeycode;
        this.fileOldName = this.arr[index].namekey;
      }
      this.namekeycode++;
