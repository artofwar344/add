<template>
	<div class="state-owned">
		...
		<div class="example-formgroup">
				...
				<Field  desc-class="richText" type="text" label="宗地编号" v-model="project_id" placeholder="请输入宗地编号" >
				</Field>
				<Field  desc-class="richText" type="text" label="项目名称" v-model="corporate_name" placeholder="请输入项目名称">
				</Field>
...
export default {
  ...
  mounted() {
    Code.findGGJNameByCode("-1").then(
      (res) => {
        ...
        console.log(this.selectData02[0].values)
      }
    )
  },
  data() {
  ...
  methods: {
    ...
    pushAjax() {
      ...
      if (this.confirmValSelect02.length != 0) {
        ...
        if (crty == "云南省") {
          ...
          if (crtys == "全部") {
            ...
            // for (var y = 0, leny = address[crty].length; y < leny; y++) {
            //   if (crtys == address[crty][y]) {
            //     this.conutycode = conuty[this.citycode][y];
            //   }
            // }
      ...
      this.$ajaxRequest("post", "/gsp/mng30043", setdata, pagejump)
        .then(res => {
