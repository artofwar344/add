...
export default {
  data: function() {
    return {
      ...
      arr: [
        {
          file_name:
            "以连续就读为由申领：学生证、就读学校出具的其他能证明连续就读的材料原件或复印件",
          content: "",
          uploadId: [],
          namekey: "_hz-13_03_abcd00090"
        },
        {
          file_name:
  ...
  methods: {
    ...
    namekeystr(index) {
      if (index < 1) {
        this.arr[index].namekey =
          ...
          "_hz-13_03_abcd00090_" +
          this.namekeycode;
        this.fileOldName = this.arr[index].namekey;
      } else {
        this.arr[index].namekey =
          sessionStorage.getItem("WorkId") +
          "_hz-13_03_abcd00085_" +
          this.namekeycode;
        this.fileOldName = this.arr[index].namekey;
      }
      this.namekeycode++;
    ...
    liveface() {
      window.sessionStorage.setItem("ApplyReasonType", "03");
      this.saveAllData();
      console.log(
