...
export default {
  ...
  mounted() {
    ...
    //       if(res[i].regnNm == "全省" || res[i].regnNm == "省本级") {
    //         continue;
    //       }
    //       this.citys[cityIndex ++] = res[i].regnNm;
    //       this.address[res[i].regnNm] = [];
    //       this.regionCode[res[i].regnCode] = res[i].regnNm;
    //       this.regionCode[res[i].regnNm] = res[i].regnCode;
    //     }
    //     this.selectData02[0].values = this.citys;
    //     for(var i = 0; i < res.length; i ++) {
    //       this.getZoneInfo(res[i].regnCode).then(
    //         (res) => {
    //           for(var i = 0; i < res.length; i ++) {
    //             this.address[this.regionCode[res[i].suprRegnCode]].push(res[i].regnNm);
    //           }
    //           this.address = Object.assign({"云南省":["全部"]}, this.address);
    //
    //         }
    //       );
    //     }
  ...
  methods: {
    ...
    pushAjax() {
      ...
      // 测试数据
      // this.corporate_name = '永平县';
      // this.citycode = '013';
      // this.conutycode = '089',
      // this.ownType = 'ckq';
      // this.calendar1.items.value = '2018-10-01';
      // this.calendar1.items.rangeValue = '2018-10-07';
      let setdata = {
        txnCommCom: {
          tRecInPage: "",
          tPageJump: "",
      ...
      console.log(setdata);
      this.$ajaxRequest("post", "/gsp/mng30039", setdata,pagejump)
        .then(res => {
