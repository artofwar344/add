    ...
    <div>
      ...
      <van-row>
        <van-col span="12">
          <div class="left-price" style="padding-bottom: 5px;">
            <div class="left-price-money">
              <span class="left-price-money-small">￥
                <span class="left-price-money-small-all">{{item.price}}.</span>00
              </span>
            </div>
        ...
        <van-col span="12">
          <div class="right-entTime" style="padding-bottom: 5px;">
            ...
            <div class="right-entTime-end" style="margin-bottom: 5px;">
              <span>{{this.d}}天</span>
              <span class="right-entTime-end-bg">{{this.hh}}</span> ：
              <span class="right-entTime-end-bg">{{this.m}}</span>
              ：
      ...
      <van-row style="margin-bottom: 4rem;">
        <van-col span="24">
          <img
            :src="imgItem"
            v-for="(imgItem,index) in dataItem.centerBanner"
            :key="index"
            v-lazy="imgItem"
            alt
            class="banner-img"
      ...
      <van-row v-show="btn1">
        <van-col span="24">
          <van-button type="warning" class="footerBtn" @click="pay()">
            ...
            <p
              class="footerBtn-btm"
              :style="item.homeBuyButtonSubheading == ''? '':item.homeBuyButtonSubheading"
            >{{item.homeBuyButtonSubheading}}</p>
          </van-button>
        </van-col>
      </van-row>
      <van-row v-show="btn2" style="margin-top: .6rem;">
        <van-col span="12" plain type="danger">
          <van-button
            ...
            @click="enterInfo()"
          >{{this.item.homeLeftButton}}</van-button>
        ...
        <van-col span="12" type="danger">
          <van-button
            ...
            @click="enterInfos()"
          >{{this.item.homeRightButton}}</van-button>
...
export default {
  ...
  created() {
    ...
    if (getCookies && getCookies !== "undefined") {
      // alert('获取用户授权成功')
      this.templateList();
    } else {
      const NowPath = window.location.href;
      console.log(NowPath, "wqeqeqewewqe");
      // alert('请先授权')
      setTimeout(function() {
        window.location =
          "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8e37f57c6f92f001&redirect_uri=" +
          NowPath +
          "&response_type=code&scope=snsapi_userinfo&state=STATE";
      }, 2000);
      // window.location = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxdcbdd6c13c897c9d&redirect_uri=http://zhongdu.natapp1.cc/zhongduPay&response_type=code&scope=snsapi_userinfo&state=STATE'
      this.userInfoCode();
  ...
  methods: {
    userInfoCode() {
      ...
      console.log(theRequest.code, "dddd");
      this.$http
        .get(
          "sns/oauth2/access_token?appid=wx8e37f57c6f92f001&secret=89738e371fef0522916e3efeb30c7692&code=" +
            theRequest.code +
        ...
        .then(res => {
          ...
          Cookies.set("slZhongDu_uc", this.userOpenid, {
            expires: 30,
            path: "/"
          });
          console.log(this.userInfoItem.refresh_token, "userInfoItem");
    ...
    userInforToken(refreshToken) {
      ...
      this.$http
        .get(
          "sns/oauth2/refresh_token?appid=wx8e37f57c6f92f001&grant_type=refresh_token&refresh_token=" +
            refreshToken,
          {}
        )
        .then(res => {
    ...
    templateList() {
      const path =
        this.$route.path + "?p=" + this.p + "&k=" + this.k + "&h=" + this.h;
      console.log(path, "login");

      if (path.indexOf("?") >= -1) {
        this.$http
          .get("/h5/newvip/indexV1?k=" + this.k + "&p=" + this.p + "&h=" + this.h)
          .then(res => {
            ...
            this.WXConfig.wxShowMenuOne(
              res.data.model.jsapiSign,
              res.data.model.shareUri,
              res.data.model.selfWxuserName,
              res.data.model.keShareTitle,
              res.data.model.keShareDesc
            );
    ...
    countdown: function() {
      this.$http
        .post("/h5/ke/difftime.do?keId=" + this.keId, {})
        .then(res => {
    ...
    pay() {
      this.$http
        .get("/h5/wx/kepay.do?keId=" + this.keId, {
    ...
    enterInfo() {
      this.$router.push({
        path: "/Hi?dataFlag=" + this.dataFlag + "&keId=" + this.keId
      });
    },
    /*  支付成功进入课程交流群按钮按钮  */
    ...
    enterInfos() {
      this.$router.push({
        path: "/Hi?dataFlags=" + this.dataFlags + "&keId=" + this.keId
      });
    },
