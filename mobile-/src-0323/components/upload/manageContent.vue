  ...
  <div class="vue-uploader">
    <div class="file-list">
      ...
      <section class="file-item" >
        <div @click="add" class="add" v-if="this.files.length <1" :style="{height: this.heights +'px'}" >
         <img  v-if=" this.EditData.imageUrl" :src="EditData.imageUrl" alt="" 
         :style="imagess" >
    ...
    <section class="upload-func">
      ...
      <div class="operation-box">
        <button v-if="status == 'ready'" @click="submitAdd">上传</button>
...
export default {
  ...
  props: { 
      EditData:  Object ,// addModel.bigImage 
      modelnimg: Object
  },
  created () {    
   this.status = 'ready'  
    ...
    submitAdd() {
      this.$http.get("api/portal/oss/token" ).then(res => { 
        ...
        for (var i = 0; i < fNum.length; i++) {
          ...
          var boxScope = 5; 
          const isWidth =this.imgWidth < this.modelnimg.width +boxScope &&this.imgWidth > this.modelnimg.width-boxScope || this.imgWidth  < this.modelnimg.width*2 +boxScope &&this.imgWidth  > this.modelnimg.width*2 -boxScope 
          const isHeight =this.imgHeight < this.modelnimg.height+boxScope &&this.imgHeight > this.modelnimg.height-boxScope || this.imgHeight  < this.modelnimg.height*2 +boxScope &&this.imgHeight  > this.modelnimg.height*2 -boxScope 

          if (!isLt2M) {
            that.$message.error("请上传2M以下的.xlsx文件");
            return false;
          }
