<template>
  <div class="reliefManagementQuery">
    <myheader :my-header="headerData"></myheader>
    <div class="reliefManagementQueryContent" id="reliefManagementQueryContent">
      ...
      <div class="reliefManagementQueryList">
          ...
          <Loadmore :top-method="loadTop" @top-status-change="handleTopChange" :bottom-method="loadBottom"
            ...
            <ul>
              <li v-for="item in ResultData" :key="item.id">
                <h4>{{item.Addr}}</h4>
                <p>机构名称：{{item.Name}}<span>{{item.Tel}}</span></p>
              </li>
            </ul>
          </Loadmore>
        </div>
...
export default {
  data () {
    return {
      ...
      allData: [
        ...
        {
          id: '03',
          address: '昆明市府前街32号，百子湾街道办事处',
          number: '0871-87000321',
          name: '社区医院'
        }
      ],
      ResultData: [],
      queryUrl: '/gsp/mng70012',
      allLoaded: false,
      bottomStatus: '',
      wrapperHeight: 0,
      topStatus: '',
  ...
  methods: {
    queryData: function () {
      let _self = this
      let listPage = document.getElementById('reliefManagementQueryContent').getElementsByClassName('hui-search_list')[0]
    ...
    filterData: function () {
      let _self = this
      return _self.allData.filter(value => {
...
<style>
  ...
  .reliefManagementQuery {
    height:100%;
  }
  .reliefManagementQueryContent{margin-top:-0.2rem;height:calc(100% - 1.1rem)}
  ...
  .reliefManagementQueryList{overflow: auto;height: calc(100% - 1.5rem)}
  .reliefManagementQueryList img{width:5.56rem;margin: 1.08rem auto;}
  .reliefManagementQueryList ul{text-align: left;}
  .reliefManagementQueryList li{margin-top: 0.16rem;background: #fff;/*height: 1.36rem;*/padding:0.26rem 0 0.22rem 0.36rem;}
  .reliefManagementQueryList h4{font-size: 0.28rem;color:#333;margin-bottom: 0.16rem;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;width: 100%;}
  .reliefManagementQueryList p{font-size: 0.24rem;color:#999;}
  .reliefManagementQueryList p:after{content: '';display: block;clear: both;}
  .reliefManagementQueryList span{float: right;padding-right:0.36rem;}
