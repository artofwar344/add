...
<template>
	<div class="state-owned">
		...
		<div class="example-formgroup">
				<Field readonly   label="地区:"  disableclear type="text" placeholder="请选择地区" :required=false :value="confirmValSelect02" @click.native="locationOpen('pickerselect02')">
				</Field>
				<HuiselectNew ref="pickerselect02" position="bottom"   :dataslots="selectData02" :visible-item-count="5" class="example-select_item" @selectval="selectChangeFun2" @confirms="selectConfirmFun02" @reset="resetconfirmValSelect02"></HuiselectNew>

				<Field  desc-class="richText" type="text" label="项目编号" v-model="project_id" placeholder="请输入项目编号" >
				</Field>
				<Field  desc-class="richText" type="text" label="公告名称" v-model="corporate_name" placeholder="请输入公告名称">
				</Field>

				<div class="example-calendar">
					<div class="example-calendar_range">
						<span class="time_title">时间段(不超过7天)</span>
						<input size="28" readonly class="example-calendar_input" type="text" @click.stop="open($event)" v-model="calendar1.items.text" placeholder="请选择时间段"/>
		...
		<!-- <div class="chargeBtn">
			<button @click="pushAjax()">查询</button>
		</div> -->
...
export default {
  ...
  methods: {
    ...
    getZoneInfo(parentCode){
      ...
      return this.$ajaxRequest("post",'/gsp/mng19021', setdata);
    },
    ...
    pushAjax() {
      ...
      this.$ajaxRequest("post", "/gsp/mng30029", setdata, pagejump)
        .then(res => {
...
.state-owned .example-select_btn-wrapper {
  margin-top: 20px;
}
.state-owned .example-select_arrowRight {
  display: block;
  width: 16px;
  height: 16px;
...
.state-owned .hui-field_core {
  text-align: right;
}
.state-owned .hui-field_title {
  width: 120px;
}
.state-owned .example-calendar {
  height: 30px;
}
/*.state-owned .example-calendar .hui-calendar {*/
  /*width: 320px;*/
  /*z-index: 1;*/
/*}*/
.state-owned .example-calendar_range {
  position: absolute;
  z-index: 1;
}
.state-owned .example-calendar_input {
  position: relative;
  z-index: 2100;
   width:50%;
  float:right;
...
.state-owned .hui-field_value .hui-field_core {
  text-align: right;
}
.state-owned .time_title {
  font-size: 0.28rem;
  float:left;
}
.state-owned .example-calendar {
