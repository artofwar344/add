  ...
  <div class="livehood-page">
    <div class="container">
      ...
      <div class="box time_service_count">
        ...
        <div class="content">
          <div class="time_service_count_g" id="time_service_count2_g" style="width: 97%; height: 430px">
  ...
  export default {
    ...
    methods: {
      ...
      mountedMethod() {
        ...
        this.ajaxWapper("/goven/hall/deparSequIndexQry", timeQuery,
          (res) => {
            this.draw_time_service_count2_g(res.data.data);
          });
      },
      // 部门事项分布 bar
      ...
      // 办事大厅概况
      hallView() {
        let accRequest ={regionId: "530000000000", regFlag: "1", totalFlag: "1", chanType: "1"};
      ...
      // 时间段服务量趋势图
      draw_time_service_count2_g(res) {
        console.log("时间段趋势图");
        console.log(res);
        // if(!res || res.length == 0){
        //   this.time_service_init();
        ...
        if(res && res.length > 0) {
          ...
          for(var i = 0; i < res.length; i ++) {
            ...
            jTb[i] = res[i].yacceSequen;
            bTb[i] = res[i].ytranSequen;
          }
        }
        let time_service_count_g = this.$echarts.init(document.getElementById('time_service_count2_g'));
            ...
            formatter: (res) => {
              ...
              for(var i = 0; i < res.length; i ++) {
                if(res[i].seriesName == "接件服务量环比" || res[i].seriesName == "办件服务量环比") {
                  rtVal += res[i].marker + res[i].seriesName + ":" + res[i].value + "%";
                }
                if(res[i].seriesName == "接件服务量同比" || res[i].seriesName == "办件服务量同比") {
                  rtVal += res[i].marker + res[i].seriesName + ":" + res[i].value + "%";
                }
                rtVal += "<br/>";
              }
            ...
            formatter: (res) => {
              ...
              for(var i = 0; i < res.length; i ++) {
                ...
                if(res[i].seriesName == "接件服务量" || res[i].seriesName == "办件服务量") {
                  rtVal += res[i].marker +
                    res[i].seriesName + ":" + res[i].value + "万件";
                }
                rtVal += "<br/>";
              }
              console.log(rtVal);
