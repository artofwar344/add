...
export default {
  ...
  methods: {
    ...
    submitEvent() {
      ...
      // 	this.$router.parent(name:'/chalist?name=' +this.name + '&textValue=' + this.textValue +'&comeReason='+this.comeReason+'&carMsg='+this.carMsg+'&citycode='+this.confirmValSelect02[0]+'&conutycode='+this.confirmValSelect02[1]+'&EntPossessionName='+this.EntPossessionName)
    },
    setEvent() {
      const _self = this;
      const cancelBtn = document.getElementsByClassName("hui-select_cancel")[0];
