      ...
      <Loadmore  :bottom-method="loadBottom"
		...
		<ul>
			<li @click='tolink(list)' class="bor_b bidding_list" v-for="list in datatext" :key="list.guid">
				...
				<p class="dis_flex font-24 margin_b16"><span class="col_999 flex_1 tx_left">项目编号：{{list.projectCode}}</span></p>
        <p class="dis_flex font-24 margin_b16"><span class="col_999 flex_1 tx_left">发布时间：{{list.publishDate}}</span></p>
...
export default {
  ...
  methods: {
    ...
    loadBottom() {
      ...
      this.$ajaxRequest("post", "/gsp/mng30033", setdata, pagejump).then((res) => {
    ...
    tolink(item) {
      ...
      this.$ajaxRequest("post", "/gsp/mng30034", setdata)
        .then(res => {
          this.$router.push({
            name: "stateTransThree",
            params: { datatext: res }
          });
        })
        .catch(error => {
