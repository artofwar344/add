      ...
      <Loadmore  :bottom-method="loadBottom"
        ...
        <ul class="list">
          <li @click='tolink(item)' :to="{name:'detailInfo',params:{detailData:item}}" v-for="item in datatext" :key="item.id">
            <h4>公告名称：{{item.winBidBulletinTitle}}</h4>
            <p>项目编号：{{item.bidSectionCode}}</p>
            <p>发布时间：{{item.winBidBulletinStartTime}}</p>
          </li>
        </ul>
      </Loadmore>
...
export default {
  ...
  methods: {
    ...
    loadBottom() {
      ...
      this.$ajaxRequest("post", "/gsp/mng30030", setdata)
        .then(res => {
          console.log(res);
          this.$router.push({
