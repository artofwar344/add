    ...
    <div class="container">
      ...
      <div class="box user_a">
        <div class="header">
          ...
          <div class="right" @click="openDate()">{{startDateValue}}
        ...
        <div class="content">
          ...
          <div class="bottom">
            <div class="tp_container" style="">
              <div class="tp_top">新增用户量</div>
              <div class="tp_bottom" style="width: 3rem">
                {{user_new_total}}
                <span class="tp_right">件</span>
              </div>
            </div>
            <div class="tp_container" style="border-left: 1px solid #F1F3F6">
              <div class="tp_top">累计用户量</div>
              <div class="tp_bottom" style="width: 3rem">
                 {{user_acc_total}}
                <span class="tp_right" >件</span>
              </div>
  ...
  export default {
    ...
    mounted () {
      ...
      this.ajaxWapper("/goven/common/nowDateQry", {},
        (res) => {
          ...
          this.user_start_date = new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 11);
          this.endDate = new Date(new Date().getTime() - 60 * 60 * 24 * 1000);
          this.mountedMethod();
        }
      )
    },
    ...
    methods: {
      ...
      handleChangeConfirmStartDate(val) {
        this.startDateValue = this.dateFormat(val, "yyyy-MM-dd");
        this.dayNewUserIndexQry(this.dateFormat(val, "yyyyMMdd"))
        console.log("handle");
        console.log(this.startDateValue);
        let userQuery = {regionId:this.regionId, regFlag:this.regFlag,chanType: "1",
          startDate:this.dateFormat(val, "yyyyMMdd"), endDate:this.dateFormat(val, "yyyyMMdd")};
      ...
      mountedMethod() {
        ...
        this.startDateValue = this.dateFormat(this.endDate, "yyyy-MM-dd");
        this.dayNewUserIndexQry(this.dateFormat(this.endDate, "yyyyMMdd"));
        // -- 提速率
        ...
        let hotQuery = {regionId : this.regionId, chanType : "1",
          regFlag : this.regFlag,  startDate: this.getMonth(0), endDate: this.getMonth(0)};
      ...
      dayNewUserIndexQry(date) {
        let query = {chanType: "1", startDate: date, endDate: date};
        this.ajaxWapper("/goven/svrBull/dayNewUserIndexQry", query,
          (res) => {
            res = res.data.data[0];
            console.log(res);
            this.user_acc_total = res.accUserTotal;
            this.user_new_total = res.newUserTotal;
          }
        )
      },
      // 第一个selector
