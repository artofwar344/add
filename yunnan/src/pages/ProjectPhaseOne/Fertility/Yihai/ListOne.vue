  ...
  <div>
	...
	v-model='emailValue'
	 placeholder="请输入婚姻变动年月"
      ...
      <!--
                <input type="text" />
                <select name="" @change="axioa('0')" v-model='sval'>
                    <option v-for="list in sheng" :key='index' :value="list.name">{{list.cent}}</option>
                </select>
                <select name="" @change="axioa('1')" v-model='svala'>
                    <option v-for="list in sheng" :key='index' :value="list">{{list}}</option>
                </select>
                <select name="" @change="axioa('2')" v-model='svals'>
                    <option v-for="list in sheng" :key='index' :value="list">{{list}}</option>
                </select>
                <select name="" @change="axioa('3')" v-model='svald'>
                    <option v-for="list in sheng" :key='index' :value="list">{{list}}</option>
                </select>
                <select name="" @change="axioa('4')" v-model='svalf'>
                    <option v-for="list in sheng" :key='index' :value="list">{{list}}</option>
      </select>-->
      <!-- </FormGroup> -->
    </div>
  ...
  methods: {
    ...
    uplod(val) {
      ...
      if (val === "-1" || val === undefined) {
        ...
        console.log(parentIds,"1")
        } else {
          parentIds = this.shi;
        console.log(parentIds,"2")
        }
        this.shi = this.quas;
        this.selectDatamap3[0].values = "";
        this.confirmValSelectmap3 = "";
    ...
    nvsub() {
      ...
      // let workid = window.sessionStorage.getItem('WorkId')
      // if (workid == null || workid == undefined || workid == "null") {
      //   this.WorkId = this.$route.query.WorkId
      //   window.sessionStorage.getItem('WorkId',this.WorkId)
      // } else {
        ...
        txnBodyCom: {
          ...
          message: {
            ...
            json_content: {
              ...
              f_hshld_rgst_lnd_ecd: this.jval, //女方户籍地编码
              f_rsdnc_lnd: this.mapValue,
              f_living_lnd: this.towMapValue,
              f_living_lnd_ecd: this.lndecd //女方现居住地编码
            }
          }
        }
      };
    ...
    //   });
      this.$ajaxRequest(
        ...
        .then(res => {
          //this.$router.push('/listoneman?WorkId='+this.WorkId)
          //this.submitEvent();
          this.isAndroidios();
          console.log(res);
        })
    ...
    submitEvent() {
      ...
      this.$ajaxRequest("post", "/gsp/mng80043", setdata)
        .then(res => {
          console.log(res);
          this.$router.push("/listoneman?work_id=" + this.WorkId);
        })
        .catch(error => {
          console.log(error);
        });
