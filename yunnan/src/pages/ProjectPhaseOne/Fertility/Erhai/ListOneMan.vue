<template>
	<div>
		...
		<div class="example-field">
			<!-- <FormGroup
      v-on:accept-submit="submitEvent"
      title-position="left"> -->
...
components: {
  ...
  methods: {
    ...
    yanzheng() {
      ...
      if (
        parseInt(this.textValue2.substr(16, 1)) % 2 == 0 ||
      ...
      this.$ajaxRequest("post","/APPS/db/insert", setdata,false,false,false,'1').then(res => {
        //this.submitEvent();
        this.$router.push('/Erlisttow?WorkId='+this.WorkId)
        console.log(res);
      })
        .catch(error => {
          console.log(error);
    ...
    submitEvent() {
      ...
      this.$ajaxRequest("post","/gsp/mng80043", setdata1).then(res => {
        console.log(res);
        //this.isAndroid_ios()
        this.$router.push('/Erlisttow?work_id='+this.WorkId)
      })
        .catch(error => {
          console.log(error);
        });
