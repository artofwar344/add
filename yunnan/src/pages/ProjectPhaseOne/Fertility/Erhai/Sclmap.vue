<template>
	...
	<div class="sclmap">
			...
			<Field readonly label="请选择地址:" desc="" disableclear type="text" placeholder="请选择" :required=false :value="confirmValSelect02" @click.native="open('pickerselect02')">
			</Field>
			<Selectpopup ref="pickerselect02" position="bottom" @touchmove.prevent  :dataslots="selectData02" :visible-item-count="5" class="example-select_item" @selectval="selectChangeFun2" @confirms="selectConfirmFun02">
			</Selectpopup>
		

		<div class="example-popup">
			<!-- <div class="example-popup_wrapper">
				<Btn plain @click.native="popupVisible5 = true" size="large" class="next">下一步</Btn>
				<Btn plain @click.native="popupVisible5 = true" size="large">确定</Btn>
			</div> -->
...
<style scoped>
	...
	.example-popup_4 .picker-slot-wrapper,
	.picker-item {
		backface-visibility: hidden;
	}
	.hui-btn_primary.is-plain{
		background: #2D7FFC;
		color: #fff;
	}
...
<script>
	...
	const address = {
		...
		//'辽宁': [{text:'沈阳',value:'02-a'},{text:'大连',value:'02-b'} ],
		...
		data() {
			return {
				...
				content: {
					...
					title: "提示标题提示标题提示标题提示标题提示标题提示标题提示标题提示标题提示标题",
					text: "段落文字段落文字段落文字段落文字段落文字段落文字段落文字段落文字段落文字段落文字段落文字",
				...
				confirmValSelect02: ["云南", "昆明"], //[ "辽宁", "大连" ]
			};
		},
...
//				this.$emit('next','1')
			...
			selectChangeFun2(picker, val) { //只有是二级的时候才有这个方法
			...
			submitBtn() {
				if(!this.confirmValSelect01[0]) { //必填校验，如果不是必填项不用写
