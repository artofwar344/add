	...
	<div>
		...
		<div class="rstbox">
			<div class="rstsosuo">
				...
				<input type="text" v-model="confirmValDate2" placeholder="请选择起始时间" @click="open('picker2')" /></span>
				<Datetime ref="picker2" type="date" v-model="valueDate" position="middle" :start-date=startDate :end-date=endDate @confirm="handleChangeConfirmDate2"></Datetime>

				<span>至</span>

				<span class="rstsosuotow"><input type="text" v-model="confirmValDate" placeholder="请选择截止时间" @click="open('picker')" /></span>
				<Datetime ref="picker" type="date" v-model="valueDate" position="middle" :start-date=startDate :end-date=endDate @confirm="handleChangeConfirmDate"></Datetime>

				<span><button @click='submitEvent(1)'>提交</button></span>
			</div>
			<div class="rsttab">
				<ul>
		...
		<div v-if="datatext != null && datatext != '' ">
			<div class="rstindcen" v-for='list in datatext.feedetail'>
				<ul >
					...
					<li><span class='srttitle'>缴费期号</span><span class='srtcent'>{{list.payment_period_number}}</span></li>
					<li><span class='srttitle'>缴费类型</span><span class='srtcent'>{{list.payment_type}}</span></li>
					<li><span class='srttitle'>补退收开始期号</span><span class='srtcent'>{{list.supply_start_number}}</span></li>
					<li><span class='srttitle'>补退收结束期号</span><span class='srtcent'>{{list.replenish_end_number}}</span></li>
					<li><span class='srttitle'>缴费基数</span><span class='srtcent'>{{list.base_pay}}</span></li>
					<li><span class='srttitle'>单位缴费金额</span><span class='srtcent'>{{list.unit_contrib_amount}}</span></li>
					<li><span class='srttitle'>个人缴费金额</span><span class='srtcent'>{{list.indie_contrib_amount}}</span></li>
					<li><span class='srttitle'>个人缴费标志</span><span class='srtcent'>{{list.personal_payment_mark}}</span></li>
					<li><span class='srttitle'>单位名称</span><span class='srtcent'>{{list.company_name}}</span></li>
				</ul>
			</div>
		</div>
...
export default {
  ...
  methods: {
    ...
    Ajaxquery(starttime, endtime) {
      ...
      let setdata = {
        ...
        txnBodyCom: {
          idno: this.cardid,
          ins_number: window.sessionStorage.getItem("EmployeeNumbers"),
      ...
      this.$ajaxRequest("post", "/gsp/mng30051", setdata)
        .then(res => {
          console.log(res);
  ...
  mounted() {
    ...
    this.endDate=new Date(this.times.getFullYear(),this.times.getMonth(),this.times.getDate())
  }
};
</script>
