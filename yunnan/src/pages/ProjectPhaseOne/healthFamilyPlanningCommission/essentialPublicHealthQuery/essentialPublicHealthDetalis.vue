  ...
  <div class="essentialPublicHealthDetalis">
    ...
    <div class="essentialPublicHealthDetalisContent" id="essentialPublicHealthDetalisContent">
      <div class="EPHMainTop">
        <img :src="itemData.logoImg" alt="">
        <div class="EPHMainTopContent">
          <div class="EPHMainTopContentLeft">
            <h4>{{itemData.name}}</h4>
            <p>{{itemData.juli}} &nbsp; | &nbsp; {{itemData.address}}</p>
          </div>
          <div class="EPHMainTopContentRight" @click="goMap(itemData)">
            <div>
      ...
      <div class="EPHMainContent">
        ...
        <ul>
          <li>
            开放时间 <span>{{itemData.time}}</span>
          </li>
          <li>
            联系方式 <span>{{itemData.phoneNum}}</span>
          </li>
          <li>
            简介 <span>{{itemData.jianjie}}</span>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  data () {
    return {
      headerData: {
        title: '国家基本公共卫生服务项目详情',
        backUrl: '/essentialPublicHealthQuery'
      },
      mapUrl: mapUrl,
      itemData: {
        name: '滇池社区卫生服务中心',
