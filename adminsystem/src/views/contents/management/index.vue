<template>
  <div class="app-container"  v-show="userFlage">
    ...
    <el-table :data="userData"  stripe style="width: 100%;height:100%; " v-if="pathView"  v-loading="MLoading"  >
...
export default {
  ...
  computed: {
    pathView() {
     this.$route.path == "/administer/activityManagement" ?(this.indexType = 3): this.$route.path == "/administer/management"  ? (this.indexType = 2)
        : (this.indexType = 1);
      ...
      return this.$route.path == "/administer/management" ||
        this.$route.path == "/administer/vipmanagement"||
         this.$route.path == "/administer/activityManagement" 
        ? true
        : false;
    }
  },
  methods: {
    ...
    createIndexModel() {
      this.indexType == 3?(this.indexMessage = "创建 活动首页模块"): this.indexType == 2 ? (this.indexMessage = "创建首页模块") : (this.indexMessage = "创建VIP首页模块");
