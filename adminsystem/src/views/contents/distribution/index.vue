    ...
    <el-card shadow='never'>
      <div class="distriClass">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="分销类型">
           ...
           <el-select v-model="formData.distributionType" placeholder="请选择"  >
            <el-option
      ...
      <div class="distriClass">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          ...
          <el-form-item>
            <el-button type="primary" @click="searchDistribution" size="small" class="nomel" :class=" nomelBtn ?'nomels':'nomelddBtn'" :disabled="nomelBtn">搜索</el-button>
      ...
      <el-table-column
        ...
        width="140" 
        label="购买用户手机">
        <!-- <template slot-scope="scope"> <span>{{new Date(Number(scope.row.buyerPhone)) }}</span></template> -->
      </el-table-column>
      <el-table-column
       width="180"
        prop="createTime" 
...
export default {
  ...
  methods: {
    ...
    subappData() {     
        ...
        this.endTime?
        this.formData.endTime = endfulltime(this.endTime) :  this.formData.beginTime = null ; 
            // console.log( this.formData ,'sddasdsfs++++++++++' );
        if(this.beginTime && this.endTime) {
        var cha =( this.endTime - this.beginTime) / 3600 / 1000 / 24;
       
          if(cha >=0 &&cha <92){ 
            //  console.log( cha,' ddddas+++++++++=' );
            // this.formData.beginTime = fulltime(this.formData.beginTime) 
   
            this.$http.post('api/portal/appDistribution/distributionExport',
            this.formData , { responseType: 'arraybuffer' }).then(resultData =>{ 
            this.download(resultData.data)
            })  
          }else if (cha >=92){
          this.$alert('时间跨度最多为3个月，请重新输入', '提示', {confirmButtonText: '确定'})
          }
        }else {
